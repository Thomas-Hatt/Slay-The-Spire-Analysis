...
namespace SpireAnalytics
{
    internal class clsData
    {
        ...
        string _strConnectionString = clsGlobal.DatabaseConnectionString;

        // Variable to link to the SQL property
        string _strSQL = "";

        // Data table for data from the database
        DataTable dtData;
        ...
        public void BatchAddRuns(List<clsRunDetail> RunsList)
        {
            ...
            using (OleDbConnection conn = new OleDbConnection(ConnectionString))
            {
                ...
                {
                    // Skip adding the row if gold is greater than 4000
                    if (run.GoldPerRun > 4000)
                    {
                        Console.WriteLine($"Skipping run with excessive gold: {run.GoldPerRun}");
                        continue;
                    }
